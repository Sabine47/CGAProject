...
class Texture2D(imageData: ByteBuffer, width: Int, height: Int, genMipMaps: Boolean): ITexture{
    ...
    companion object {
        ...
        operator fun invoke(path: String, genMipMaps: Boolean): Texture2D {
            ...
            val imageData: ByteBuffer = STBImage.stbi_load(path, x, y, readChannels, 4)
    ...
    override fun processTexture(imageData: ByteBuffer, width: Int, height: Int, genMipMaps: Boolean) {
        //1 Textur ID Generieren
        texID = glGenTextures(); GLError.checkThrow()

        //2 Textur aktivieren
        glBindTexture(GL_TEXTURE_2D, texID); GLError.checkThrow()

        //3 Textur definieren und laden
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, imageData); GLError.checkThrow()

        //4 Mipmaps generieren (optional)
        if(genMipMaps)
    ...
    override fun setTexParams(wrapS: Int, wrapT: Int, minFilter: Int, magFilter: Int) {
        ...
        GL11.glTexParameterf(GL_TEXTURE_2D, EXTTextureFilterAnisotropic.GL_TEXTURE_MAX_ANISOTROPY_EXT, 16.0f)
        unbind()
    }
